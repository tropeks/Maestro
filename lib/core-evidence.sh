...
_ev_read_vars() { # <arquivo> → linhas "e_x=valor" (aspas simples), só campos validados — caller faz eval "$(...)"
  ...
  awk -F= 'NR>20 { exit }
    ...
    /^inconclusive=/ { if ($2 ~ /^[0-9]+$/)          print "e_inc="  $2 }' "$f" 2>/dev/null
  return 0
}

# ------------------------------------------------------------------- escrita
_ev_write() { # <arquivo> <label> <cmd_hash> <exit> <wtree_before> <wtree_after> <cmd_match> <load1m_x100> <ncpu> <inconclusive> → grava (tmp+mv); rc 0/2
